...
class SoccerNetClips(Dataset):
    def __init__(self, path, features="ResNET_PCA512.npy", split=["train"], version=1, 
        ...
        # salvar para cada clip qual o seu vídeo correspondente
        # ou seja, se o primeiro jogo X tiver 200 clips,
        #  então X_clips[0:200] é o path para o vídeo X
        self.game_index = []

        # salvar para cada clip qual o seu evento correspondente
        # dentro do seu respectivo jogo
        # ou seja, se o clip X é o segundo clip do jogo Y, então
        # self.clip_index[X] = 2
        self.clip_index = []
        ...
        for game in tqdm(self.listGames):
            ...
            n_clips_1 = feat_half1.shape[0]
            n_clips_2 = feat_half2.shape[0]

            # n_labels_1 = label_half1.shape[0]
            # n_labels_2 = label_half2.shape[0]

            path_1 = os.path.join(self.path, game, "1_" + self.features)
            path_2 = os.path.join(self.path, game, "2_" + self.features)

            n_path_1 = [path_1]*n_clips_1
            n_path_2 = [path_2]*n_clips_2

            # save the path for each game
            self.game_index = self.game_index + n_path_1 + n_path_2
            # save the index of each clip
            self.clip_index = self.clip_index + list(range(n_clips_1)) + list(range(n_clips_2))
        
        # concatenate das labels apenas - apenas labels sendo salvas na memória
    ...
    def __getitem__(self, index):
        ...
        """
        game = self.game_index[index]
        print('game path: ', game)
        # Load features
        feat = np.load(game)
        feat = feat.reshape(-1, feat.shape[-1])    
        feat = feats2clip(torch.from_numpy(feat), stride=self.window_size_frame, clip_length=self.window_size_frame)
        # print('features shape: ', feat.shape)

        clip_index = self.clip_index[index]
        clip = feat[clip_index, :, :]
        # print('clip shape: ', clip.shape)

        clip_label = self.game_labels[index, :]
        # print('clip label: ', clip_label)
